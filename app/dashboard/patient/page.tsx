...
import { Activity, Calendar, FileText, Pill, Users, LogOut, CreditCard } from 'lucide-react'
  ...
  const fetchPatientData = async () => {
    try {
      ...
      if (patientResponse.ok) {
        ...
        // Find the patient that matches the current user
        const currentUserPatient = patientData.patients.find(
          patient => patient.userId === session.user.id
        )
        setPatientData(currentUserPatient)
      }
              ...
              <div className="space-y-4">
                {medicalRecords.slice(0, 3).map((record) => (
                  <div key={record._id} className="border-l-4 border-green-500 pl-4 py-2">
                    ...
                    <p className="text-gray-600 text-sm">Dr. {record.doctor?.firstName} {record.doctor?.lastName}</p>
